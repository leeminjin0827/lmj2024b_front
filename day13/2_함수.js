    ...
    - 함수 , function
        ...
        2. 함수 종류
            ...
            2. 만들어져 있는 함수 : 우리(개발자)에게 빠른 개발이 가능 하도록 도구 제공( 라이브러리 )
                - console.log( ) , documen.write( ) , .push( ) , .splice( ) 등등
        ...
        4. JS 함수 표현 방법
            1. 선언적 합수 : 선언 키워드 이용하여 함수를 정의하는 방법 , function 함수명( ){ }
        ...
        5. js 함수 만드는 방법
            ...
            3. ( 매게변수1, 매게변수 ) : 인(들어올 인)수(숫자) : 함수가 실행될때 함수 안으로 들어오는 데이터
            4. { }              : 함수가 실행될때 처리할 명령어들의 구역
            5. return           : 함수가 처리하고 함수가 호출했던 곳으로 반환 되는 데이터 , 리턴값 , 반환값

        6. 함수 호출 하는 방법
            함수명( 인자값 , 인자값 ); : 인자값이 각 함수의 매게변수에 대입되고 함수의 실행후 return 반환 한다.
        ...
        8. 함수 사용 특징
            1. 코드의 재 활용/사용
            2. 미리 정의된 코드의 인자값(매게변수)에 따른 서로 다른 결과(리턴값) 얻을수 있다.
*/
...
function 함수1차( a , x , b ){ // 매게변수란? 함수 호출시 함수 안에 대입할 인자값 저장하는 변수
    console.log( a ) // 1   // 10
    console.log( x ) // 2   // 20
    console.log( b ) // 3   // 5
...
// [4] 
let y1 = 함수1차( 1 , 2 , 3); // 대입값만 다르고 함수라 처리하는 공식/명령어 동일
let y2 = 함수1차( 10 , 20 , 5 ); // 대입값만 다르고 함수가 처리하는 공식/명령어 동일

// [5] 믹서기 함수 : 과일 갈아서 만든 쥬스
function 믹서기함수( 과일 ){
    // 처리
    let 결과 = 과일+'주스'
    return 결과;
