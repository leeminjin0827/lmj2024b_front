/* 
    객체 : 여러가지 자료를 하나의 자료 타입 구조
        ...
        2.(미리존재한)남이만든객체 : document , console , new Date() 등등
            -> 남이 미리 데이터/함수 제공 함으로써 바른개발이 가능하다 ( 도구 제공 )
            -> 내가 집을 만드는데 도구는 사서 사용한다. ( 라이브러리 )
    
    세션 과 쿠키
        - 브라우저( HTML/CSS/JS 코드를 렌더링/그려주는 프로그램 , HTTP 통신 )
        - 브라우저 F12(개발자도구) -> [NETWORK] -> 왼쪽 사이드 메뉴 ( local storge(쿠키) / session storge(세션) )

        1. 세션 : 사용자가 웹페이지를 방문하고 있는 동안 메모리를 저장 하는 공간 , 브라우저 꺼지면 데이터 삭제
            ex] 검색엔진 , 임시데이터
            객체 : sessinStorge

        2. 쿠키 : 사용자가 웹페이지를 방문이 끝나도 메모리는 저장 하는 공간 , 브라우저 꺼져도 메모리 유지 된다. ( 사용자가 직접 삭제 )
            ex] 자동로그인 기능 구현
            객체 : localStorge
            
            SessinStorge /  localStorge 객체의 속성/기능 제공  
...
// [2] 세션 / 쿠키 데이터 삽임 , 키 와 값 을 한쌍으로 하는 데이터 구조
sessionStorage.setItem( 'name' , '유재석' ); // F12개발자 -> [APPLICATIPN] 탭 -> 왼쪽메뉴 session storge 클릭
localStorage.setItem( 'age' , 40 ); // F12개발자 -> [APPLICATIPN] 탭 -> 왼쪽메뉴 local storge 클릭

// [3] 세션 / 쿠키 데이터 호출 , 키를 이용한 값 호울
...
// sessionStorage.setItem( '할일' , { 할일내용 : "밥먹기" , 할일상태 : true } ); // 객체를 저장할수 없다. 
