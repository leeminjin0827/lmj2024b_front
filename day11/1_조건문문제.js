...
// 1.[입력/저장]
    // let 정수1 = Number( prompt('[3] 정수 1 : ')) ;
    // let 정수2 = Number( prompt('[3] 정수 2 : ')) ;
    // let 정수3 = Number( prompt('[3] 정수 3 : ')) ;
    // let max = 정수1 // 천번째 값을 가장큰값(max) 으로 선정
    // if( max < 정수2 ){ max = 정수2; } // 만약에 두번째 값이 가장큰값 보다 크면 가장큰값(max)을 두번재값으로 대입
    // if( max < 정수3 ){ max = 정수3; } // 만약에 세번째 값이 가장큰값 보다 크면 가장큰값(max)을 세번재값으로 대입
    // console.log(`[3] 결과 : ${max} `)
        // if vs if ~ else
...
// [ 1.정렬/비교 ]
    // (오름차순)
    // A와 B 비교 해서 B가 더 작으면 B를 A와B 값 교체 A<--->B
        // if( a < b ){ }
    // A와 C 비교 해 C가 더 작으면 C를 A와C 값 교체 A<--->C
        // if( a > c ){ }
    // B와 C 비교 해서 C가 더 작으면 C를 B와C 값 교체 B<--->C
        // if( b > c ){ } 
    // (내림차순) : 부등호 반대 , 더 크면
    
// [ 2.변수 값 교환/swap ]
    // - 한번에 2개 연산을 동시에 처리할수 없다. 무조건 연산은 1개씩 처리한다.
    // - 변수는 무조건 데이터/자료 1개만 저장 ...정수1.
    // - 주의할점  A = B ; A변수에 B 값 를 대입하면 가존의 A값은 사라진다.
    //            A = B; B = A; // - 컴퓨터 순차처리 이므로 A값은 이미 사라졌다.
    // - swap 방법
        // 임시변수 temp 황용
        // let temp = A; A = B; B = temp;

    // let 정수1 = Number( prompt('[4] 정수 1 : ')) ;
    // let 정수2 = Number( prompt('[4] 정수 2 : ')) ;
    // let 정수3 = Number( prompt('[4] 정수 3 : ')) ;
    ...
    // console.log( `[4] 결과 : ${ 정수1 } ${ 정수2 } ${ 정수3 }`)
        // - 다수의 데이터를 비교 할때는 if 가 많아진다. ----> '반복문' 활용
...
// 5. 1개의 점수를 입력받아 90점 이상이면 'A등급' 80점이상이면 'B등급' 70점이상 'C등급' 그외 '재시험' 출력하시오.
    ...
    // 주의할점 : if fi if vs if ~else if ~ else if
...
// 아니면 배열에 입력받은 과일명 저장 하고 현재 배열을 출력하시오.
    ...
    // // .indexOf( '찾을값' ) : 지정한 찾을값이 배열내 존재하면 찾은인덱스변환 없으면 -1 변환
    // // .push( 새로운값 ) : 지정한 새로운값을 배열내 마지막 인덱스 요소의 추가

    // // 만약에 존재여부가 -1이 아니면(존재하지않으면) 배열내 입력받은 과일명 추가
...
// 8. 주민등록번호를 입력받아서 성별이 남자이면 색상변수에 'blue' 을 여자이면 'red' 을 대입 하고 색상변수 출력하시오.
    ...
    // // - 문자 = [ 'a' , 'b' , 'c' ] , 문자 - [ '유' , '재' , '석' ]
    // // - JS는 문자열을 배열로 취급하기 때문에 배열 함수를 사용할수 있다. [ 인덱스 사용 가능하다. ]
    // console.log( 주민등록번호 [8] ) // 800420-1234567 -> [0]8 [1]0 [2]0 [3]4 [4]2 [5]0 [6]1 [7]2 [8]3 [9]4 [10]5
    // if( 주민등록번호[7] == 1 || 주민등록번호[7] == 3){
    //     // 만약에 주민등록번호의 8번째자리(7인덱스) 값이 1또는 3이면 남자
    //     색상변수 = 'blue'
    // }else{ // 아니면
    //     색상변수 = 'red'
    // }
...
// 비겼을경우 '무승부' 출력 하시오.
    ...
    // // [방법1]
    // if( (p1 == 0 && p2 == 2) || (p1 == 1 && p2 == 0) || (p1 == 2 && p1 == 1 ) ){
...
//     if( sheetList[ 예약좌석번호 -1 ] == 'O'){  // -1 를 하는 이유 ? 배열 길이(데이터수)는 1~3 , 인덱스는0~2
