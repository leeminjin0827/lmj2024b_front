    ...
    for( let index = 0 ; index<=사원목록.length-1 ; index++ ){  // - 사원목록내 모든 사원정보를 HTML로 구성하기 
        ...
        html += `<tr>
                    ...
                    <td> 
                        ...
                        <button onclick="평가결과()" type="button">평가결과</butoon>
                    </td>
                </tr>`
    } // f end 
    // 3. 출력 
...
function 평가등록출력( 클릭된사원번호 ){ // 매개변수 : 클릭된사원번호
    ...
    let html = `<h3> 사원 평가 등록 </h3>
                <form>
                    ...
                    상반기평가 : <input class="upPoint"type="text" />
                    하반기평가 : <input class="dnPoint"type="text"/> 
                    <button onclick="평가등록()" type="button"> 점수 등록 </button>
                </form>`
    // 3, 출력 
    div.innerHTML = html;
} // f end 
function 평가등록(){
    let year = document.querySelector('.year').value;   // console.log(year);
    let up = document.querySelector('.upPoint').value;  // console.log(up);
    let dn = document.querySelector('.dnPoint').value;  // console.log(dn);
    let info = {
        사원번호 : eno ,
        연도 : year ,
        상반기 : up , 
...
// 평가결과();
function 평가결과() {
    let tbody = document.querySelector('.평가테이블');
    let html = ``;
...
//         - 상하 관계 , 일 대 다 관계 , 1개 데이터가 다른곳에서 여러개를 참조한다.
//             - 하나의 사원이 여러개 연도별 점수를 가질수 있다.
//                 '유재석' -------->2024   100 0-
//                          -------->2024   85 64
//                     일     대     디
//             - 하나의 회원이 여러개의 게시물울 작성할수 있다.
//             - 하나의 카테고리의 여러 제품을 등록할수 있다.

//     3. 데이터 분류 간의 관계성 연걸
//         - 사원정보(상위) , 점수정보(하위)
//         1. 유재석 정보 등록 ,유재석 2002-10-02
//         2. 유재석 2024점수 등록 , 유재석 2002-10-02 2024 090 80
//         3. 유재석 2023점수 등록 , 유재석 2002-10-02 2024 90 80- 2023 100 97
//             유재석 2002-10-02 ----> 2024  90 -00
//                               ----> 2023 100 97
//             1. 유재석 2002-10-02 ----> 유재석 2024 90 80
//                                 ----> 유재석 2023 100 97
// */
// // [1] 객체지향으로 메모리 구성
//     // 1. 두 사원의 정보(no:사원정보,name:사원명,bitrh:생년월일)를 샘플로 해서 2개의 객체 생성
// let 사원1 = { no : 1 , name : '유재석' , birth : '2000-10-2' };
// let 사원2 = { no : 2 , name : '강호동' , birth : '1998-08-18'};
//     // 2. 사원목록 등록
// let 사원목록 = []
// 사원목록.push( 사원1 ); // 사원1 등록
// 사원목록.push( 사원2 ); // 사원2 등록
...
// // [2] 사원 정보 전체 출력 함수 , 실행조건 : 1.JS실행될때최초1번 2.사원목록(배열) 변화(저장/수정/삭제)가 있을때 마다
// 사원전체출력();
// function 사원전체출력(){
...
//     for( let index  = 0 ; index <= 사원목록.length - 1 ; index++){ // index는 0부터 사원목록 마지막 인덱스까지 1씩 증가 반복 
//         let info = 사원목록[index]; // index번째의 사원정보 1개 호출
//         html += `<tr>
//                     <td> ${ info.사원번호 } </td>
//                     <td> ${ info.사원명 } </td>
...
// function 평가등록출력( 클릭된사원번호 ){ // 매개변수 : 클릭된사원번호
//     // 1. 클릭된사원번호를 이용한 사원정보 찾기.
//     console.log( 클릭된사원번호 ); // 2.'검색사원' 변수 선언하여 검색 결과를 저장할 변수 // null 객체가 없다는 뜻
//     let 검색사원 = null; // 검색사원을 찾기 전 변수
//     for( let index = 0 ; index <= 사원목록.length - 1 ; index++ ){ // 3. index는 0부터 마지막인덱스까지 1씩 증가 반복
//         let info = 사원번호[index]; // 4. index번째의 사원정보(객체) 호출
