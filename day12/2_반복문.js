...
// 반복되지않는코드: 0 1 2 // 반복되지 않는코드 순서/패턴 : 0부터 2까지 1씩 증가
// [반복문코드]
for( let index = 0 ; index <= 2 ; index++ ){ // index는 0부터 2까지 1씩증가반복
    console.log( 과일상자[ index ] )
} // for end
for( let index = 0 ; index <= 과일상자.length - 1 ; index++){ // index는 0부터 마지막인덱스까지 1씩증가반복
    console.log( 과일상자[ index ] )
} // for end
...
// prompt(' 점수1 : ') // (1)HTML에서 알람창에서 입력받은 값을 반환 해주는 함수
...
let 점수3 = prompt('점수3 : ')
점수배열.push( 점수3 );
// 중복코드 : let 점수? = prompt('점수? : ') , 점수배열.push( 점수? );
...
// [반복문 코드] 반복코드 : outHTML += '<li> ${ 점수배열[0] } </li>
let outHTML2 = '<ol>'
for( let index = 0 ; index <= 점수배열2.length - 1 ; index++ ){ // index는 0부터 마지막인덱스 까지 1씩 증가 반복
