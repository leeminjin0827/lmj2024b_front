...
for( let i = 1 ; i <= 10 ; i++ ){ // i는 1부터 10이하까지 1씩증가 반복 한다.
...
// (실습1) 6개의 점수를 입력받아 하나의 배열에 저장하시오.
    // - 조건1 : 배열내 중복은 없어야 한다.
    // - 조건2 : 배열내 개수가 6개 되면 프로그램 종료 한다. 

let numArray = [] ; // 입력받은 데이터를 저장하는 배열 선언
for( ; ; ){ // 무한루프 start // 무한루프 종료조건 : 배열내 요소가 6개이면 종료
    // (1) 무한입력
    let num = Number( prompt(` ${ numArray.length } 번쨰 수 입력 : `) )
    // (2) 처리 : 중복검사  ,배열저장 , 개수체크
